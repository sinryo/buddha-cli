            ...
            "capabilities": {
                ...
                "prompts": {
                    "low-token-guide": {
                        ...
                        "messages": [
                            {"role": "system", "content": "Prefer low-token flow: 0) If corpus/ID is unknown, call resolve({query}) and follow its recommended fetch tool+args. 1) If ID is known, call fetch directly (source auto-detected from ID). 2) Otherwise use search({source, query}), read _meta.fetchSuggestions, then call fetch with {id, lineNumber, contextBefore:1, contextAfter:3, highlight:<query>, format:\"plain\"}. 3) Use pipeline only for cross-file summary; set autoFetch=false by default."}
                        ]
                    }
                },
                "logging": {}
...
fn tools_list() -> Vec<serde_json::Value> {
    if crate::unified::is_unified_mode() {
        return crate::unified::unified_tools_list();
    }
    legacy_tools_list()
}
...
fn legacy_tools_list() -> Vec<serde_json::Value> {
    vec![
        ...
        tool(
            "sat_detail",
            "Fetch SAT detail by useid. Use sat_search to get useid (startid) then call sat_detail/sat_fetch to retrieve text.",
            json!({"type":"object","properties":{"useid":{"type":"string"},"key":{"type":"string"},"startChar":{"type":"number"},"maxChars":{"type":"number"}},"required":["useid"]}),
        ),
        tool(
            "sat_fetch",
            "Fetch SAT page text (prefer useid over URL). Use sat_search to get useid (startid).",
        ...
        tool(
            "sat_pipeline",
            "SAT pipeline: search (wrap7) -> pick best hit -> auto-fetch detail text. Use when you want the best matching text content immediately (single best).",
            json!({"type":"object","properties":{
            "query":{"type":"string"},
            ...
            "maxChars":{"type":"number"}
		        ...
		        tool("sat_search", "Search SAT Taisho Shinshu Daizokyo text database (online). Returns _meta.results (raw docs) AND _meta.fetchSuggestions (use sat_detail/sat_fetch with useid). Tip: filter by Taisho id range using fq (e.g., fq:[\"id:[T2000 TO T2920]\"] for Japanese compositions T56-T85).", json!({"type":"object","properties":{
		            "query":{"type":"string"},
		            "rows":{"type":"number"},
		            "offs":{"type":"number"},
		            "exact":{"type":"boolean"},
...
/// Combined info handler for info({section:"all"}): version + usage + system_prompt
fn handle_info_all(id: serde_json::Value) -> serde_json::Value {
    ...
    let version_text = version_resp
        ...
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let usage_text = usage_resp
        .pointer("/result/content/0/text")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    let prompt_text = prompt_resp
        .pointer("/result/content/0/text")
        .and_then(|v| v.as_str())
        .unwrap_or("");
    ...
    let combined = format!(
        "=== Version ===\n{}\n\n=== Usage Guide ===\n{}\n\n=== System Prompt ===\n{}",
        version_text, usage_text, prompt_text
    );
    ...
    json!({
        ...
        "result": {
            ...
            "_meta": {"source": "info_all"}
        }
    })
}

fn handle_call(id: serde_json::Value, params: &serde_json::Value) -> serde_json::Value {
    let name_raw = params.get("name").and_then(|v| v.as_str()).unwrap_or("");
    let args_raw = params.get("arguments").cloned().unwrap_or(json!({}));

    // Unified dispatch: translate unified tool names to legacy names
    let (name, args) =
        if crate::unified::is_unified_mode() && crate::unified::is_unified_tool(name_raw) {
            let (legacy_name, legacy_args) = crate::unified::dispatch_unified(name_raw, &args_raw);
            // Handle _error from dispatch
            if legacy_name == "_error" {
                let msg = legacy_args
                    .get("message")
                    .and_then(|v| v.as_str())
            ...
            // Handle _info_all (combined info)
            if legacy_name == "_info_all" {
                let mut resp = handle_info_all(id);
                crate::unified::rewrite_meta_suggestions(&mut resp);
                return resp;
            }
            (legacy_name, legacy_args)
        } else {
            (name_raw.to_string(), args_raw)
        };
...
If you know the Taisho number, use fetch with id directly:
...
Example: fetch({id: "T0262"}) - instant! (source auto-detected)

### Tipitaka (Pāli Canon)
Use Nikāya codes directly with fetch:
...
Examples:
- fetch({id: "DN1"}) - Brahmajāla Sutta (梵網経)
- fetch({id: "MN1"}) - Mūlapariyāya Sutta
- fetch({id: "s0101m.mul"}) - Direct file access

### GRETIL (Sanskrit TEI)
Use file stem directly with fetch:
...
Examples:
- fetch({source: "gretil", id: "saddharmapuNDarIka"}) - Lotus Sutra Sanskrit
- fetch({source: "gretil", id: "vajracchedikA"}) - Diamond Sutra Sanskrit
- fetch({source: "gretil", id: "sa_azvaghoSa-buddhacarita"}) - with sa_ prefix also works

### SARIT (TEI P5)
Use file stem directly with fetch:
- fetch({source: "sarit", id: "asvaghosa-buddhacarita"})

### MUKTABODHA (Sanskrit library; local files)
Use file stem directly with fetch:
- fetch({source: "muktabodha", id: "<file-stem>"})

## Online sources (no local data needed)

### SAT Database (online)
Flow: search({source:"sat"}) -> fetch with useid
- search({source: "sat", query: "法華経", exact: true})
- fetch({useid: "..."})  # use startid/useid from search results

### Jodo Shu Zensho (浄土宗全書, online)
Flow: search({source:"jozen"}) -> fetch with lineno
- search({source: "jozen", query: "薬師"})
- fetch({lineno: "J01_0200B19"})
...
- Use resolve({query: "法華経"}) first, then follow the recommended fetch calls.

## Standard flow (when ID unknown)
1. Use search({source, query}) -> read _meta.fetchSuggestions
2. Call fetch with {id, lineNumber, contextBefore:1, contextAfter:3, highlight:"検索語", format:"plain"}
3. IMPORTANT: Always include 'highlight' parameter with the search term!
4. Use pipeline only for multi-file summary; set autoFetch=false by default
...
"#;
        ...
        "sat_search" => {
            ...
            // sat_search formatting and suggestions rely on title + useid.
            let fields_used =
                sat_wrap7_ensure_fields(fields_requested, &["id", "fascnm", "startid", "endid"]);
            ...
            if let Some(jsonv) = sat_wrap7_search_json(&q_param, rows, offs, &fields_used, &fq) {
                ...
                let count = jsonv
                    ...
                    .and_then(|v| v.as_u64())
                    .unwrap_or(0);
                let mut docs_enriched: Vec<serde_json::Value> = Vec::new();
                for d in docs_raw.iter() {
                    if let Some(obj) = d.as_object() {
                        ...
                        let useid = o
                            ...
                            .unwrap_or("")
                            .to_string();
                        if let Some(tn) = sat_tnumber_from_startid(&useid) {
                            o.entry("tNumber".to_string())
                                .or_insert_with(|| serde_json::Value::String(tn));
                        }
                        docs_enriched.push(serde_json::Value::Object(o));
                    } else {
                        docs_enriched.push(d.clone());
                    }
                }
                ...
                for d in docs_enriched.iter().take(hint_top) {
                    let useid = d
                        .get("startid")
                        .and_then(|v| v.as_str())
                        .unwrap_or("")
                ...
                if auto {
                    ...
                    let mut summary = if titles_only {
                        format!(
                            "{} titles (showing {}-{}):\n\n",
                    ...
                    for (i, d) in docs.iter().enumerate() {
                        summary.push_str(&format!(
                            "{}. {}\n",
                            shown_start + i,
                            sat_doc_display_line(d)
                        ));
                    }
                    summary.push_str(
	                        "\nSee _meta.results for raw docs and _meta.fetchSuggestions for next-step fetch calls.",
	                    );
                    return json!({ "jsonrpc":"2.0","id": id, "result": { "content": [{"type":"text","text": summary }], "_meta": meta_base }});
                }
            } else {
...
fn sat_tnumber_from_startid(useid: &str) -> Option<String> {
    // SAT useid/startid examples: "2203A,57,0011a01" -> "T2203A"
    let p0 = useid.split(',').next()?.trim();
    if p0.is_empty() {
        return None;
    }
    Some(format!("T{}", p0))
...
fn sat_doc_display_line(doc: &serde_json::Value) -> String {
    let title = doc.get("fascnm").and_then(|v| v.as_str()).unwrap_or("");
    let useid = doc.get("startid").and_then(|v| v.as_str()).unwrap_or("");
    let tnum = doc
        .get("tNumber")
        .and_then(|v| v.as_str())
        .map(|s| s.to_string())
        .or_else(|| sat_tnumber_from_startid(useid))
        .unwrap_or_else(|| "?".to_string());
    if title.is_empty() {
        format!("{} [useid: {}]", tnum, useid)
    } else {
        format!("{} {} [useid: {}]", tnum, title, useid)
    }
}
...
mod tibetan_tests {
    ...
    #[test]
    fn test_sat_tnumber_from_startid() {
        assert_eq!(
            sat_tnumber_from_startid("2203A,57,0011a01").as_deref(),
            Some("T2203A")
        );
        assert_eq!(sat_tnumber_from_startid("").as_deref(), None);
        assert_eq!(sat_tnumber_from_startid(",57,0011a01").as_deref(), None);
    }
...
"#;
        ...
        assert_eq!(
            d.line_ids,
            vec!["J01_0200A01".to_string(), "J01_0200B19".to_string()]
        );
